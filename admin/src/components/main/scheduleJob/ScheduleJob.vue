    ...
    >
      ...
      <!--   <FormItem prop="jobId" label="业务ID:">
        ...
        <FormItem prop="type" label="业务类型:">
          <Select v-model="addScheduleJob.type" transfer size="large" style="width:200px">
              <Option v-for="item in typeList" :value="item.id" :key="item.id">{{ item.value }}</Option>
          </Select>
        </FormItem>
        <FormItem prop="status" label="状态:">
          <Select v-model="addScheduleJob.jobStatus" transfer size="large" style="width:200px">
    ...
    >
      <Form ref="updateScheduleJob" :model="updateScheduleJob" :label-width="100" label-position="right"  :rules="updateScheduleJobRules">
       ...
       <!--  <FormItem prop="jobId" label="业务ID:">
        ...
        <FormItem prop="type" label="业务类型:">
          <Select v-model="updateScheduleJob.type" transfer size="large" style="width:200px">
              <Option v-for="item in typeList" :value="item.id" :key="item.id">{{ item.value }}</Option>
          </Select>
        </FormItem>
        <FormItem prop="status" label="状态:">
          <Select v-model="updateScheduleJob.jobStatus" transfer size="large" style="width:200px">
              <Option v-for="item in jobStatusList" :value="item.id" :key="item.id">{{ item.value }}</Option>
          </Select>
        </FormItem>
      </Form>
...
export default {
  ...
  data () {
    return {
        ...
        typeList:[
			...
			addScheduleJobRules: {
                /* jobId: [
                    {required: true, message: '工作任务业务Id为必填项', trigger: 'blur'}
                    ], */
                },
			addScheduleJob:{
			},
      ...
      deleteScheduleJob:{},
        ...
        },
        {
          ...
          align:'center',
          render: (h, params) => {
              let jobIdvalue=params.row.jobId;
            if(jobIdvalue==0){
              jobIdvalue="自动全部";
            }
             return  h('span',jobIdvalue);
          }
        },
        {
        	title:'业务类型',
  ...
  //增加
	 add (params) {
      ...
      this.addScheduleJob.type=1;
      this.addScheduleJob.jobId=0;//0代表所有
    },
		//增加取消
		 addCancel () {
      if (!this.addLoading) {
